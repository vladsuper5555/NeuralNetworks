...
def processData(dataset):
    np_dataset_images = np.array([object[0] for object in dataset], dtype = np.uint8).reshape(len(dataset), 28 * 28)
...
def runEpoch(batch_size=100):
    ...
    for batchIndex in range(batch_count):
        ...
        for i, layer in enumerate(layers):
            ...
            layer.weights -= LEARNING_RATE * (gradients_accumulated[i] + L2_REGULARIZATION * layer.weights)
